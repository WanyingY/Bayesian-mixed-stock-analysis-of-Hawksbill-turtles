...
nesting_data <- data.frame(
  area = c("N.&C.Am.", "S.Am", "Southeast Asia waters", "Colombian Pacific Park"),
  nests = c(484, 68, 112, 9)
)

coordinates <- data.frame(
  area = c("N.&C.Am.", "S.Am", "Southeast Asia waters", "Colombian Pacific Park", "Hainan"),
  lat  = c(20, -2, 10, 4, 20),
  lon  = c(-110, -79, 120, -77, 110)
)
...
nesting_data$population_abundance_prior <- nesting_data$nests / sum(nesting_data$nests)
...
p <- ggplot(plot_data, aes(x = area, y = combined_prior)) +
  ...
  geom_point(size = 4, shape = 21, fill = "white", color = "black", stroke = 1.2) +
  labs(x = "", y = "Estimated Source Contribution", title = "") +
